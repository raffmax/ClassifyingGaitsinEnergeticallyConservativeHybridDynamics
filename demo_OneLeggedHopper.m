...
%   It uses pseudo-arclength continuation in the exploration of the
%   connected component.
%
...
x_init = [0; l0; 0; l0; 0; -g*tF; 0; -g*tF]; 
...
% NOTE: We do not need an anchor constraint here, since all gaits are
% implemented to start at touch down. Thus, a phase shift is not possible.
% An implementation with an anchor constraint at apex transit during flight
% (like in the paper) would lead to the same results but is less 
% computational efficient, since it requires the numerical integration of 
% 3 modes (flight,stance,flight).
...
fixedStates             = 1; % fixed at initial state
% define the states which need to be periodic
periodicStates          = (2:8);

% set analytic first order gradients
opts.Grad1 = true; 
...
[LC_M6,LC_M5] = arcPC.branchingOff(LCBif2);
...
%% extract data from connected component
...
grid on
hold on
plot([M0.E0;BP1.Sol.param.E0],[M0.x0(:,5);BP1.Sol.x0(5)],'b')
plot([BP1.Sol.param.E0;M1.E0;BP2.Sol.param.E0],[BP1.Sol.x0(5);M1.x0(:,5);BP2.Sol.x0(5)],'g')
plot([BP1.Sol.param.E0;M2.E0],[BP1.Sol.x0(5);M2.x0(:,5)],'r')
plot([BP1.Sol.param.E0;M3.E0],[BP1.Sol.x0(5);M3.x0(:,5)],'r--')
plot([BP2.Sol.param.E0;M4.E0],[BP2.Sol.x0(5);M4.x0(:,5)],'c')
plot([BP2.Sol.param.E0;M5.E0],[BP2.Sol.x0(5);M5.x0(:,5)],'k')
plot([BP2.Sol.param.E0;M6.E0],[BP2.Sol.x0(5);M6.x0(:,5)],'k--')
xlabel('$\bar{E}$','Interpreter','latex')
ylabel('$\dot{x}_0$','Interpreter','latex')
scatter(BP1.Sol.param.E0,BP1.Sol.x0(5),'k','filled')
scatter(BP2.Sol.param.E0,BP2.Sol.x0(5),'k','filled')
legend('$\mathcal{M}_0$','$\mathcal{M}_1$','$\mathcal{M}_2$','$\mathcal{M}_3$',...
       '$\mathcal{M}_4$','$\mathcal{M}_5$','$\mathcal{M}_6$','BC1','BC2','Interpreter','latex')
%% create animations for gaits on different generators at E_bar = 1.8
